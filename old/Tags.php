...
<div class="menu">
<ul>
<li class="active"><a href="explore.php">Explore All</a></li>
...
<section class="Collage effect-parent">
    <?php
        $off = $page*20-19;
        if ($cat!='none'){
        $query = "select u.id, u.url, u.width, u.height "
                . "from Url u join Common c on u.id=c.p_id "
                . "where c.nsfw=0 "
                . "and u.width is not null and u.height is not null "
                . "limit 20 offset $off";
        }elseif($cat='nsfw'){
        $query = "SELECT u.id, u.url, u.width, u.height FROM Url u, Common c"
                . "AND c.nsfw=1 AND c.p_id=u.id "
                . "and u.width is not null and u.height is not null";
        }
        else{
        $query = "SELECT u.id, u.url, u.width, u.height FROM Url u, Common c, TagRelation tr, Tag t "
                . "WHERE t.tagName like '%".$cat."%' AND t.id=tr.tagid "
